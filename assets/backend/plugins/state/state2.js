...
$(function() {
    ...
    $('#states').html(options);
    $("#districts").html('<option value="NO VALUE">DISTRICT</option>');
    $("#states").on("change",function(){select_district( $("#states").val());});
});
function select_district($val)
{
    if($val=='') {
        var options = '<option value="NO VALUE">DISTRICT</option>';
